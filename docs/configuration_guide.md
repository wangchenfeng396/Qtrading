...
| `POSITION_SIZE_PCT` | `0.20` | **单笔资金占用 (20%)**<br>双重约束：每笔交易投入的本金不得超过总资金的 20%。<br>*最终仓位 = min(风险计算量, 资金占用量)* |
| `MAX_OPEN_POSITIONS`| `4` | **最大持仓数量**<br>配合 `POSITION_SIZE_PCT` 使用，总占用 = 20% * 4 = 80%，预留 20% 缓冲。 |
